...
public class Paging {
	...
	protected final static String BASE = ApplicationConstant.base+"/home?currentPage=";
	protected final static String BASE_ACCOUNT = ApplicationConstant.base+"/account/list?currentPage="; 
	/**
	 * 
	 * <ul>
	 ...
	 * @return
	 */
	public static String pagingScriptAccount(int currentPage,int pageSize,int tottlePage){
		StringBuilder sb = new StringBuilder();
		sb.append("<ul>");
		sb.append("<li "+(currentPage != 0 ? "" :"class=\"disabled\"")+"><a href=\""+BASE_ACCOUNT+"0\" title=\"点击去到第一页\">&laquo; 第一页</a></li>");
		if(0 <= currentPage && currentPage < tottlePage-1) sb.append(" <li><a href=\""+BASE_ACCOUNT+(currentPage + 1)+"\" title=\"下一页\">&rarr;</a></li>");
		
		if(tottlePage >= 0 && tottlePage <= showNum){
			//直接显示
			sb.append(addTosb(0,tottlePage,currentPage));
		}else{
			if(currentPage < showNum / 2){
				sb.append(addTosb(tottlePage / 2 - showNum / 2 , tottlePage / 2 - 1,currentPage));
				
				sb.append("<li><a href=\""+BASE_ACCOUNT+(currentPage+1)+"\">..</a></li>");
				
				sb.append(addTosb(tottlePage / 2, tottlePage-2,currentPage));
			}
			if(currentPage > showNum / 2 && currentPage < (tottlePage - showNum / 2)){
				sb.append(addTosb(currentPage - showNum / 2, currentPage-1,currentPage));
				
				sb.append("<li><a href=\""+BASE_ACCOUNT+(currentPage+1)+"\">..</a></li>");
				
				sb.append(addTosb(currentPage ,currentPage + showNum / 2,currentPage));
			}
			if(currentPage > (tottlePage - showNum / 2)){
				sb.append(addTosb(tottlePage / 2 - showNum / 2, tottlePage / 2,currentPage));
		...
		if(currentPage > 0 && currentPage < tottlePage) sb.append(" <li><a href=\""+BASE_ACCOUNT+(currentPage-1)+"\" title=\"下一页\">&rarr;</a></li>");
		if(currentPage > 0 && currentPage < tottlePage) sb.append("<li "+(currentPage != (tottlePage-1) ? "" :"class=\"disabled\"")+"><a href=\""+BASE_ACCOUNT+(tottlePage-1)+"\" title=\"跳转到最后一页\">最后一页 &raquo;</a></li>");
		sb.append("<ul>");
		return sb.toString();
	}
	...
	private static String addTosb(int begain,int end,int current){
		StringBuilder sb = new StringBuilder();
		for(int i=begain;i<end;i++){
			if(i == current) continue;
			sb.append("<li><a href=\""+BASE_ACCOUNT+i+"\">"+(i+1)+"</a></li>");
		}
		return sb.toString();
	}
